            ...
            <div class="kt-portlet__head">
                ...
                <div class="kt-portlet__head-toolbar">
                    <div class="kt-portlet__head-actions">
                        <a class="btn btn-outline-secondary btn-bold btn-sm @if ($type === "all") active @endif" href="{{route("overtime_approval_list",["type"=>"all"])}}">
                            Tümü
                        </a>
                        <a class="btn btn-outline-secondary btn-bold btn-sm @if ($type === "manager") active @endif" href="{{route("overtime_approval_list",["type"=>"manager"])}}">
                            Yönetici Onayı
                        </a>
                        <a class="btn btn-outline-secondary btn-bold btn-sm @if ($type === "hr") active @endif" href="{{route("overtime_approval_list",["type"=>"hr"])}}">
                            IK Onayı
                        </a>
                        <a class="btn btn-outline-secondary btn-bold btn-sm @if ($type === "document") active @endif" href="{{route("overtime_approval_list",["type"=>"document"])}}">
                            Belge Onayı
                        </a>
                        <a class="btn btn-outline-secondary btn-bold btn-sm @if ($type === "success") active @endif" href="{{route("overtime_approval_list",["type"=>"success"])}}">
                            Tamamlandı
                        </a>
                    </div>
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                <tr @if ($type != "success" && $type != "all") hidden @endif>
                                    <td>1</td>
                                    <td><a class="kt-link" href="{{route("overtime_approval_view",["overtime_id"=>1])}}">Serkan Erdinç</a></td>
